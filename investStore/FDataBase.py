...
class FDataBase:
    ...
    def addStatement(self, id_user, id_status, id_car, booking_date):
        try:
            self.__cur.execute(
                "INSERT INTO request VALUES(NULL, ?, ?, ?, ?)",
                (id_user, id_status, id_car, booking_date),
            )
            self.__db.commit()
    ...
    def getAllStatements(self):
        try:
            self.__cur.execute("""SELECT request.*,
                               user.*
                                FROM request
    ...
    def addUser(self, name, email, psw, phone, driver_license):
        try:
            self.__cur.execute(
            ...
            self.__cur.execute(
                ...
                (psw, name, phone, email, driver_license),
            )
            self.__db.commit()
        except sqlite3.Error as e:
    ...
    def getCars(self):
        try:
            self.__cur.execute(f"SELECT * FROM car")
            res = self.__cur.fetchall()
            if res:
                return res
        except sqlite3.Error as e:
